class GroupResController < ApplicationController
  ...
  def new
    ...
    unless Campground.open?(@reservation.startdate, @reservation.enddate)
      debug 'not open'
      flash[:warning] = I18n.t('reservation.Flash.ClosedDates', :closed => DateFmt.format_date(@option.closed_start), :open => DateFmt.format_date(@option.closed_end))
  ...
  def find_spaces
    ...
    @season = Season.find_by_date(@reservation.startdate).id
  ...
  def spaces_selected
    ...
    if num_selected == 0
      ...
      redirect_to :controller => 'group', :action => 'find'
    end
  end
  ...
  def create
    ...
    # this is called from camper so the parameters contain a camper id
  ...
  def checkin
    ...
    reservations = Reservation.all(:conditions => ["group_id = ? and checked_in = ? and archived = ?", res.group_id, false, false])
  ...
  def checkout
    ...
    camper = Camper.update(res.camper_id, :activity => currentDate)
    reservations = Reservation.all(:conditions => ["group_id = ? and checked_in = ? and archived = ?", res.group_id, true, false])
