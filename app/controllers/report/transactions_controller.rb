class Report::TransactionsController < ApplicationController
  ...
  def create
    if params[:csv]
      ...
      csv_str = '"Occurance","Reservation","Space","Camper","Start date","End date",'
      csv_str << '"Discount",' if discounts > 0
      csv_str << '"Recommender",' if recommendations
      csv_str << "\"Description\"\n"
      @new_res.each do |res|
	st_ = res.startdate.strftime("%m/%d/%Y")
	end_ = res.enddate.strftime("%m/%d/%Y")
	csv_str << "\"New\",\"#{res.id}\",\"#{res.space.name}\",\"#{res.camper.full_name}\",#{st_},#{end_ },"
	csv_str << (res.discount_id > 0 ? "#{res.discount.name}," : '"",') if discounts > 0
	csv_str << (res.recommender_id > 0 ? "#{res.recommender.name}," : '"",') if recommendations
	csv_str << "\"#{res.last_log_entry('reservation')}\"\n"
      end
      @checkins.each do |res|
	st_ = res.startdate.strftime("%m/%d/%Y")
	end_ = res.enddate.strftime("%m/%d/%Y")
	csv_str << "\"Check-in\",\"#{res.id}\",\"#{res.space.name}\",\"#{res.camper.full_name}\",#{st_},#{end_ },"
	csv_str << (res.discount_id > 0 ? "#{res.discount.name}," : '"",') if discounts > 0
	csv_str << (res.recommender_id > 0 ? "#{res.recommender.name}," : '"",') if recommendations
	csv_str << "\"#{res.last_log_entry('checkin')}\"\n"
      end
      @checkouts.each do |res|
	st_ = res.startdate.strftime("%m/%d/%Y")
	end_ = res.enddate.strftime("%m/%d/%Y")
	csv_str << "\"Check-out\",\"#{res.id}\",\"#{res.space.name}\",\"#{res.camper.full_name}\",#{st_},#{end_ },"
	csv_str << (res.discount_id > 0 ? "#{res.discount.name}," : '"",') if discounts > 0
	csv_str << (res.recommender_id > 0 ? "#{res.recommender.name}," : '"",') if recommendations
	csv_str << "\"#{res.last_log_entry('checkout')}\"\n"
      end
      @cancels.each do |res|
	st_ = res.startdate.strftime("%m/%d/%Y")
	end_ = res.enddate.strftime("%m/%d/%Y")
	csv_str << "\"Cancel\",\"#{res.id}\",\"#{res.space.name}\",\"#{res.camper.full_name}\",#{st_},#{end_ },"
	csv_str << (res.discount_id > 0 ? "#{res.discount.name}," : '"",') if discounts > 0
	csv_str << (res.recommender_id > 0 ? "#{res.recommender.name}," : '"",') if recommendations
	csv_str << "\"#{res.last_log_entry('cancel')}\"\n"
      end
      @changes.each do |res|
	st_ = res.startdate.strftime("%m/%d/%Y")
	end_ = res.enddate.strftime("%m/%d/%Y")
	...
	change_array.each do |c|
	  csv_str << "\"Change\",\"#{res.id}\",\"#{res.space.name}\",\"#{res.camper.full_name}\",#{st_},#{end_ },"
	  csv_str << (res.discount_id > 0 ? "#{res.discount.name}," : '"",') if discounts > 0
	  csv_str << (res.recommender_id > 0 ? "#{res.recommender.name}," : '"",') if recommendations
	  csv_str << "\"#{c}\"\n"
	end
	...
	@page_title = "Transactions for #{DateFmt.format_date(@res.startdate)}"
      else
	@page_title = "Transactions for #{DateFmt.format_date(@res.startdate)} to #{DateFmt.format_date(@res.enddate)}"
      end
