...
#####################################################
  ...
  def update_colors
	...
	logger.debug "updating #{c[:name]} with #{c[:value]}"
    ...
    colors = "#{RAILS_ROOT}/app/views/setup/color/colors.erb"
    colors_css = "#{RAILS_ROOT}/public/stylesheets/colors.css"
    color_string = render_to_string(:layout => false, :file => colors)
    File.delete(colors_css)
    file = File.new(colors_css, "w+")
  ...
  def update
    ...
    @color = Color.find(params[:id])
    logger.debug "changes #{@color.name} from #{@color.value} to #{params[:color]}"
    colors = "#{RAILS_ROOT}/app/views/setup/color/colors.erb"
    logger.debug "color file is #{colors}"
    colors_css = "#{RAILS_ROOT}/public/stylesheets/colors.css"
    logger.debug "output file is #{colors_css}"
