module CamperHelper
  ...
  def next_action(camper)
    case session[:next_controller]
    when 'reservation'
      case session[:next_action]
      when 'update_camper'
	# logger.debug 'reservation#update_camper'
	"<a href=\"/reservation/update_camper?camper_id=#{camper}&amp;reservation_id=#{@reservation_id}\">#{I18n.t('general.Select')}</a>"

      when 'confirm_res'
	# logger.debug 'reservation#confirm_res'
	"<a href=\"/reservation/confirm_res?camper_id=#{camper}&amp;reservation_id=#{@reservation_id}\">#{I18n.t('general.Select')}</a>"
      else
        # not complete
