module Setup::PriceHelper
  ...
  def all_rates
	...
	if rate
      ...
      ret_str << "<td>" +  link_to('Delete', { :action => 'destroy', :id => p.id },
  ...
  def show_rate(rate, season_id, price_id)
	...
	button_to  sprintf(fmt, rate), {:action => :edit_1, :season_id => season_id, :price_id => price_id}
      else
	button_to number_2_currency(rate), {:action => :edit_1, :season_id => season_id, :price_id => price_id}
      end
    else
      button_to 'none', {:action => :edit_1, :season_id => season_id, :price_id => price_id}
    end
  end
end
