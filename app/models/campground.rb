...
# a class for things concerning the campground
  ...
  def self.closed_dates(thisdate)
    option = Option.first

    c_start = Date.new(thisdate.year, option.closed_start.month, option.closed_start.day)
    c_end = Date.new(thisdate.year, option.closed_end.month, option.closed_end.day)

    if option.closed_start < option.closed_end # closed all in one calendar year eg. closed for summer like in AZ
      if thisdate < c_end
        # before the closure
	closed = false
      elsif c_end < thisdate
        # after the closure
	c_start = Date.new(thisdate.year+1, option.closed_start.month, option.closed_start.day)
	c_end = Date.new(thisdate.year+1, option.closed_end.month, option.closed_end.day)
	closed = false
      else
        # currently closed
	c_start = Date.new(thisdate.year+1, option.closed_start.month, option.closed_start.day)
	closed = true
      ...
      # closure spans parts of two calendar years eg. closed for winter like in MN.
      if thisdate < c_end # 1
	# before opening initial start and end are current
	closed = true
      elsif thisdate > c_start # 3
	# after closure
	c_start = Date.new(thisdate.year+1, option.closed_start.month, option.closed_start.day)
	c_end = Date.new(thisdate.year+1, option.closed_end.month, option.closed_end.day)
	closed = true
