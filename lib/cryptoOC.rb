module CryptoOC
  class Button
    def initialize(data)
      my_cert_file = RAILS_ROOT + "/config/paypal/my-pubcert.pem"
      my_key_file = RAILS_ROOT + "/config/paypal/my-prvkey.pem"
      if ENV['RAILS_ENV'] == "production"
	paypal_cert_file = RAILS_ROOT + "/config/paypal/paypal_cert.pem"
      else
	paypal_cert_file = RAILS_ROOT + "/config/paypal/sandbox_cert.pem"
      end
